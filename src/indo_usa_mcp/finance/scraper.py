"""OpenStreetMap Overpass scraper for Indian-American finance & tax.

Matches accountant/tax/financial offices whose name signals an Indian-from-India firm (an Indian
surname or explicit Indian marker). We require the Indian token (bare "Tax"/"CPA" matches
everything) to keep the India-diaspora guardrail tight. Public, ODbL, no login. Sparse + noisy —
...
# Indian-from-India name signal: common surnames + explicit Indian markers. Combined with the
# finance office keys this yields Indian-name CPAs / tax / financial advisors.
_NAMES = ("Indian|India |Patel|Shah|Sharma|Gupta|Singh|Reddy|Desai|Mehta|Nair|Iyer|Khanna|"
          ...
          "Trivedi|Saxena|Sinha|Chauhan|Bansal|Punjabi|Gujarati|Telugu|Tamil|Bharat|Hindustan")
_KEYS = [("office", "accountant|tax_advisor|financial_advisor|financial")]
...
(
  node["office"~"accountant|tax_advisor|financial_advisor|financial"]["name"~"Indian|India |Patel|Shah|Sharma|Gupta|Singh|Reddy|Desai|Khanna|Malhotra|Punjabi|Gujarati",i](area.usa);
  way["office"~"accountant|tax_advisor|financial_advisor|financial"]["name"~"Indian|India |Patel|Shah|Sharma|Gupta|Singh|Reddy|Desai|Khanna|Malhotra|Punjabi|Gujarati",i](area.usa);
);
out center tags;
"""


class FinanceOverpassScraper:
    ...
    def _to_candidate(self, element: dict, region: str) -> dict | None:
        ...
        return {
            ...
            "finance_type": tags.get("office") or "accountant",
