...
def get_indian_groceries(
    *, lat: float | None = None, lng: float | None = None, radius_miles: float = 15.0,
    ...
    """List active Indian grocery stores by geo-radius and/or filters (incl. tag, open_now)."""
    ...
    sql = (f"SELECT {_COLS_SQL} FROM groceries WHERE {' AND '.join(where)} "
...
def search_groceries_by_text(
    ...
    sql = (f"SELECT {_COLS_SQL}, similarity(name, %s) AS match_score FROM groceries "
