"""Real-estate pipeline: scrape -> raw -> clean/enrich/score -> canonical -> versioning."""
...
from .scraper import RealEstateOverpassScraper
...
def _realestate_type(name: str, raw_type: str | None) -> str:
    n = (name or "").lower()
    if any(k in n for k in ("mortgage", "loan", "lending")):
        return "mortgage"
    if any(k in n for k in ("realty", "realtor", "real estate", "properties", "homes",
                            "associates", "group", "brokerage")):
        return "agency"
    return raw_type or "agent"
...
def clean_realestate(c: dict) -> dict:
    ...
    rec = {
        ...
        "realestate_type": _realestate_type(name, c.get("realestate_type")),
    ...
    rec["tags"] = sorted(set(tagmod.extract("realestate", rec)) | set(c.get("extra_tags") or []))
    rec["description"] = describe.describe("realestate", rec)
...
def scrape_to_raw(region: str) -> int:
    count = 0
    for c in RealEstateOverpassScraper().scrape(region):
        db.execute(
            "INSERT INTO realestate_raw (source_name, source_url, source_id, payload) "
...
def process_raw() -> dict[str, int]:
    ...
    for row in db.query("SELECT id, payload FROM realestate_raw WHERE NOT processed ORDER BY id"):
        rec = clean_realestate(_as_dict(row["payload"]))
        stats[_reconcile(rec)] += 1
        stats["processed"] += 1
        db.execute("UPDATE realestate_raw SET processed = true, processed_at = now() WHERE id = %s",
                   (row["id"],))
    return stats


def _reconcile(rec: dict) -> str:
    existing = db.query_one("SELECT * FROM realestate WHERE natural_key = %s", (rec["natural_key"],))
    if existing is None:
        _insert(rec)
        return "inserted"
    if not existing["is_active"] and not existing["is_claimed"] and rclean.is_publishable(rec):
        db.execute("UPDATE realestate SET is_active = true, updated_at = now() WHERE id = %s",
                   (existing["id"],))
    diff = {f: rec[f] for f in _DIFF_FIELDS
            if rec.get(f) not in (None, "", [], {}) and _normalize(existing.get(f)) != _normalize(rec.get(f))}
    if not diff:
        db.execute("UPDATE realestate SET last_seen_at = now() WHERE id = %s", (existing["id"],))
        return "unchanged"
...
def _insert(rec: dict) -> None:
    ...
    row = db.query_one(
        f"INSERT INTO realestate ({', '.join(cols)}, is_active, last_seen_at) "
...
def _update(existing: dict, rec: dict, diff: dict) -> None:
    ...
    row = db.query_one(
        f"UPDATE realestate SET {assignments}, version = %s, updated_at = now(), "
...
def _snapshot(row: dict, reason: str) -> None:
    db.execute(
        "INSERT INTO realestate_versions (realestate_id, version, data, change_reason) "
        "VALUES (%s, %s, %s, %s)",
        (row["id"], row["version"], Jsonb(_jsonable(row)), reason),
    )
    if embeddings.enabled():
        db.execute("UPDATE realestate SET embedding = %s::vector WHERE id = %s",
...
def deactivate_stale(days: int = 180) -> dict[str, int]:
    rows = db.query(
        "SELECT id FROM realestate WHERE deleted_at IS NULL AND is_active "
        "AND NOT is_claimed AND last_seen_at < now() - (%s || ' days')::interval", (days,))
    for r in rows:
        db.execute("UPDATE realestate SET is_active = false, version = version + 1, "
