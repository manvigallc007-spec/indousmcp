"""Studio pipeline: scrape -> raw -> clean/enrich/score -> canonical -> versioning."""
...
from .scraper import StudioOverpassScraper

_CANONICAL_FIELDS = [
    "natural_key", "name", "address_full", "city", "state", "country", "lat", "lng",
    "phone", "email", "website", "hours_json", "studio_type", "region_tag",
...
def clean_studio(c: dict) -> dict:
    ...
    rec = {
        ...
        "studio_type": _studio_type(name, c.get("studio_type")),
        "region_tag": None,
    ...
    rec["tags"] = sorted(set(tagmod.extract("studios", rec)) | set(c.get("extra_tags") or []))
    rec["description"] = describe.describe("studios", rec)
    rec["confidence_score"] = _score(rec)
    return rec


def _studio_type(name: str, raw_type: str | None) -> str | None:
    """Normalize the discipline from the name (more useful than the raw OSM tag)."""
    n = (name or "").lower()
    if "yoga" in n:
        return "yoga"
    if any(k in n for k in ("bharatanatyam", "kathak", "kuchipudi", "odissi", "nritya",
                            "natyam", "dance", "nataraj")):
        return "dance"
    if any(k in n for k in ("tabla", "sitar", "veena", "mridangam", "carnatic",
                            "hindustani", "sangeet", "music", "raga")):
        return "music"
    return raw_type
...
def scrape_to_raw(region: str) -> int:
    count = 0
    for c in StudioOverpassScraper().scrape(region):
        db.execute(
            "INSERT INTO studio_raw (source_name, source_url, source_id, payload) "
...
def process_raw() -> dict[str, int]:
    ...
    for row in db.query("SELECT id, payload FROM studio_raw WHERE NOT processed ORDER BY id"):
        rec = clean_studio(_as_dict(row["payload"]))
        stats[_reconcile(rec)] += 1
        stats["processed"] += 1
        db.execute("UPDATE studio_raw SET processed = true, processed_at = now() WHERE id = %s",
                   (row["id"],))
    return stats


def _reconcile(rec: dict) -> str:
    existing = db.query_one("SELECT * FROM studios WHERE natural_key = %s", (rec["natural_key"],))
    if existing is None:
        _insert(rec)
        return "inserted"
    if not existing["is_active"] and not existing["is_claimed"]:
        db.execute("UPDATE studios SET is_active = true, updated_at = now() WHERE id = %s",
                   (existing["id"],))
    diff = {f: rec[f] for f in _DIFF_FIELDS
            if rec.get(f) not in (None, "", [], {}) and _normalize(existing.get(f)) != _normalize(rec.get(f))}
    if not diff:
        db.execute("UPDATE studios SET last_seen_at = now() WHERE id = %s", (existing["id"],))
        return "unchanged"
...
def _insert(rec: dict) -> None:
    ...
    row = db.query_one(
        f"INSERT INTO studios ({', '.join(cols)}, last_seen_at) "
...
def _update(existing: dict, rec: dict, diff: dict) -> None:
    ...
    row = db.query_one(
        f"UPDATE studios SET {assignments}, version = %s, updated_at = now(), "
...
def _snapshot(row: dict, reason: str) -> None:
    db.execute(
        "INSERT INTO studio_versions (studio_id, version, data, change_reason) "
        "VALUES (%s, %s, %s, %s)",
        (row["id"], row["version"], Jsonb(_jsonable(row)), reason),
    )
    if embeddings.enabled():
        db.execute("UPDATE studios SET embedding = %s::vector WHERE id = %s",
...
def deactivate_stale(days: int = 150) -> dict[str, int]:
    rows = db.query(
        "SELECT id FROM studios WHERE deleted_at IS NULL AND is_active "
        "AND NOT is_claimed AND last_seen_at < now() - (%s || ' days')::interval", (days,))
    for r in rows:
        db.execute("UPDATE studios SET is_active = false, version = version + 1, "
