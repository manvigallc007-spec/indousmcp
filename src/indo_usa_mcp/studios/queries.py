"""Read queries backing the studio MCP capabilities."""
...
def get_indian_studios(
    *, lat: float | None = None, lng: float | None = None, radius_miles: float = 15.0,
    ...
    """List active Indian yoga/dance/music studios, ranked by proximity + freshness + Featured."""
    point = (lat, lng) if lat is not None and lng is not None else None
    return ranking.geo_list("studios", _COLS_SQL, point=point, city=city, state=state,
                            tag=tag, open_now=open_now, limit=limit, offset=offset, radius_miles=radius_miles)


def get_studio_details(studio_id: int) -> dict[str, Any] | None:
    record = db.query_one(
        f"SELECT {_COLS_SQL} FROM studios WHERE id = %s AND deleted_at IS NULL", (studio_id,))
    if record is None:
        return None
    record["version_history"] = db.query(
        "SELECT version, change_reason, changed_at FROM studio_versions "
        "WHERE studio_id = %s ORDER BY version DESC", (studio_id,))
    return record
...
def search_studios_by_text(
    ...
    return ranking.text_search("studios", _COLS_SQL, query_text, city=city, state=state,
...
def stats() -> dict[str, Any]:
    ...
    return {
        "raw_total": scalar("SELECT count(*) FROM studio_raw"),
        "raw_unprocessed": scalar("SELECT count(*) FROM studio_raw WHERE NOT processed"),
        "studios_active": scalar("SELECT count(*) FROM studios WHERE deleted_at IS NULL AND is_active"),
        "versions_total": scalar("SELECT count(*) FROM studio_versions"),
        "cities": db.query(
            "SELECT city, state, count(*) AS n FROM studios WHERE deleted_at IS NULL "
