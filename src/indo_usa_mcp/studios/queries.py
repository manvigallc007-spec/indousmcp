...
def get_indian_studios(
    *, lat: float | None = None, lng: float | None = None, radius_miles: float = 15.0,
    ...
    """List active Indian yoga/dance/music studios by geo-radius and/or filters."""
    ...
    sql = (f"SELECT {_COLS_SQL} FROM studios WHERE {' AND '.join(where)} "
...
def search_studios_by_text(
    ...
    sql = (f"SELECT {_COLS_SQL}, similarity(name, %s) AS match_score FROM studios "
