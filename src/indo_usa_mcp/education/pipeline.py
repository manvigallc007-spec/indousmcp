"""Education pipeline: scrape -> raw -> clean/enrich/score -> canonical -> versioning."""
...
from .scraper import EducationOverpassScraper
...
_REGIONS = ("Telugu", "Tamil", "Gujarati", "Marathi", "Bengali", "Kannada", "Malayalee",
            "Malayali", "Punjabi", "Hindi", "Odia", "Konkani", "Sindhi", "Sanskrit")


def _edu_type(name: str, raw_type: str | None) -> str:
    n = (name or "").lower()
    if any(k in n for k in ("language", "telugu", "tamil", "hindi", "gujarati", "kannada",
                            "marathi", "bengali", "sanskrit", "vidyalaya", "patashala", "pathshala")):
        return "language_school"
    if any(k in n for k in ("bal vihar", "balvihar", "chinmaya", "gurukul", "heritage", "veda",
                            "vedic", "bharatiya", "sunday school")):
        return "heritage"
    if any(k in n for k in ("coaching", "academy", "learning", "tutor", "prep", "institute")):
        return "tutoring"
    return raw_type or "school"
...
def clean_education(c: dict) -> dict:
    ...
    rec = {
        ...
        "edu_type": _edu_type(name, c.get("edu_type")),
    ...
    rec["tags"] = sorted(set(tagmod.extract("education", rec)) | set(c.get("extra_tags") or []))
    rec["description"] = describe.describe("education", rec)
...
def scrape_to_raw(region: str) -> int:
    count = 0
    for c in EducationOverpassScraper().scrape(region):
        db.execute(
            "INSERT INTO education_raw (source_name, source_url, source_id, payload) "
...
def process_raw() -> dict[str, int]:
    ...
    for row in db.query("SELECT id, payload FROM education_raw WHERE NOT processed ORDER BY id"):
        rec = clean_education(_as_dict(row["payload"]))
        stats[_reconcile(rec)] += 1
        stats["processed"] += 1
        db.execute("UPDATE education_raw SET processed = true, processed_at = now() WHERE id = %s",
                   (row["id"],))
    return stats


def _reconcile(rec: dict) -> str:
    existing = db.query_one("SELECT * FROM education WHERE natural_key = %s", (rec["natural_key"],))
    if existing is None:
        _insert(rec)
        return "inserted"
    ...
    if not diff:
        db.execute("UPDATE education SET last_seen_at = now() WHERE id = %s", (existing["id"],))
        return "unchanged"
...
def _update(existing: dict, rec: dict, diff: dict) -> None:
    ...
    row = db.query_one(
        f"UPDATE education SET {assignments}, version = %s, updated_at = now(), "
...
def _snapshot(row: dict, reason: str) -> None:
    db.execute(
        "INSERT INTO education_versions (education_id, version, data, change_reason) "
        "VALUES (%s, %s, %s, %s)",
        (row["id"], row["version"], Jsonb(_jsonable(row)), reason),
    )
    if embeddings.enabled():
        db.execute("UPDATE education SET embedding = %s::vector WHERE id = %s",
...
def deactivate_stale(days: int = 180) -> dict[str, int]:
    rows = db.query(
        "SELECT id FROM education WHERE deleted_at IS NULL AND is_active "
        "AND NOT is_claimed AND last_seen_at < now() - (%s || ' days')::interval", (days,))
    for r in rows:
        db.execute("UPDATE education SET is_active = false, version = version + 1, "
