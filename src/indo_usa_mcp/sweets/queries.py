...
def get_indian_sweets(
    *, lat: float | None = None, lng: float | None = None, radius_miles: float = 15.0,
    ...
    """List active Indian sweets shops / bakeries by geo-radius and/or filters."""
    ...
    sql = (f"SELECT {_COLS_SQL} FROM sweets WHERE {' AND '.join(where)} "
...
def search_sweets_by_text(
    ...
    sql = (f"SELECT {_COLS_SQL}, similarity(name, %s) AS match_score FROM sweets "
