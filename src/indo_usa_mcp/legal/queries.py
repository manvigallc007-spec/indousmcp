"""Read queries backing the immigration & legal MCP capabilities."""
...
def get_indian_legal(
    *, lat: float | None = None, lng: float | None = None, radius_miles: float = 25.0,
    ...
    """List active Indian-American immigration attorneys & law firms, ranked by proximity +
    freshness + Featured (filter by `tag`, e.g. 'immigration')."""
    point = (lat, lng) if lat is not None and lng is not None else None
    return ranking.geo_list("legal", _COLS_SQL, point=point, city=city, state=state,
                            tag=tag, open_now=open_now, limit=limit, offset=offset, radius_miles=radius_miles)


def get_legal_details(legal_id: int) -> dict[str, Any] | None:
    record = db.query_one(
        f"SELECT {_COLS_SQL} FROM legal WHERE id = %s AND deleted_at IS NULL", (legal_id,))
    if record is None:
        return None
    record["version_history"] = db.query(
        "SELECT version, change_reason, changed_at FROM legal_versions "
        "WHERE legal_id = %s ORDER BY version DESC", (legal_id,))
    return record
...
def search_legal_by_text(
    ...
    return ranking.text_search("legal", _COLS_SQL, query_text, city=city, state=state,
...
def stats() -> dict[str, Any]:
    ...
    return {
        "raw_total": scalar("SELECT count(*) FROM legal_raw"),
        "raw_unprocessed": scalar("SELECT count(*) FROM legal_raw WHERE NOT processed"),
        "legal_active": scalar("SELECT count(*) FROM legal WHERE deleted_at IS NULL AND is_active"),
        "versions_total": scalar("SELECT count(*) FROM legal_versions"),
        "cities": db.query(
            "SELECT city, state, count(*) AS n FROM legal WHERE deleted_at IS NULL "
