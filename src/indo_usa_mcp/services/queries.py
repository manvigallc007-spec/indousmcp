...
def get_indian_services(
    *, lat: float | None = None, lng: float | None = None, radius_miles: float = 15.0,
    ...
    """List active Indian community services, ranked by proximity + freshness + Featured."""
    point = (lat, lng) if lat is not None and lng is not None else None
    return ranking.geo_list("services", _COLS_SQL, point=point, city=city, state=state,
...
def search_services_by_text(
    ...
    return ranking.text_search("services", _COLS_SQL, query_text, city=city, state=state,
