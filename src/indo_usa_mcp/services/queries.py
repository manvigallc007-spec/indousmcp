"""Read queries backing the services MCP capabilities."""
...
def get_indian_services(
    *, lat: float | None = None, lng: float | None = None, radius_miles: float = 15.0,
    ...
    """List active Indian community services (money transfer, banks, immigration, travel)."""
    ...
    sql = (f"SELECT {_COLS_SQL} FROM services WHERE {' AND '.join(where)} "
...
def get_service_details(service_id: int) -> dict[str, Any] | None:
    record = db.query_one(
        f"SELECT {_COLS_SQL} FROM services WHERE id = %s AND deleted_at IS NULL", (service_id,))
    if record is None:
        return None
    record["version_history"] = db.query(
        "SELECT version, change_reason, changed_at FROM service_versions "
        "WHERE service_id = %s ORDER BY version DESC", (service_id,))
    return record
...
def search_services_by_text(
    ...
    sql = (f"SELECT {_COLS_SQL}, similarity(name, %s) AS match_score FROM services "
...
def stats() -> dict[str, Any]:
    ...
    return {
        "raw_total": scalar("SELECT count(*) FROM service_raw"),
        "raw_unprocessed": scalar("SELECT count(*) FROM service_raw WHERE NOT processed"),
        "services_active": scalar("SELECT count(*) FROM services WHERE deleted_at IS NULL AND is_active"),
        "versions_total": scalar("SELECT count(*) FROM service_versions"),
        "cities": db.query(
            "SELECT city, state, count(*) AS n FROM services WHERE deleted_at IS NULL "
