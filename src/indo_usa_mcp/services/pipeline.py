"""Service pipeline: scrape -> raw -> clean/enrich/score -> canonical -> versioning."""
...
from .scraper import ServiceOverpassScraper

_CANONICAL_FIELDS = [
    "natural_key", "name", "address_full", "city", "state", "country", "lat", "lng",
    "phone", "email", "website", "hours_json", "service_type", "region_tag",
...
def clean_service(c: dict) -> dict:
    ...
    rec = {
        ...
        "service_type": _service_type(name, c.get("service_type")),
        "region_tag": None,
    ...
    rec["tags"] = sorted(set(tagmod.extract("services", rec)) | set(c.get("extra_tags") or []))
    rec["description"] = describe.describe("services", rec)
    rec["confidence_score"] = _score(rec)
    return rec


def _service_type(name: str, raw_type: str | None) -> str | None:
    """Normalize the service category from the name (clearer than the raw OSM tag)."""
    n = (name or "").lower()
    if any(k in n for k in ("remit", "money transfer", "money2india", "xpress money",
                            "ria money", "forex", "currency exchange", "western union")):
        return "money_transfer"
    if any(k in n for k in ("immigration", "visa", "green card")):
        return "immigration"
    if any(k in n for k in ("travel", "yatra", "tours", "akbar", "riya")):
        return "travel"
    if "insurance" in n:
        return "insurance"
    if any(k in n for k in ("tax", "cpa", "accountant")):
        return "tax"
    if raw_type in ("bank", "bureau_de_change"):
        return "bank" if raw_type == "bank" else "money_transfer"
    return raw_type
...
def scrape_to_raw(region: str) -> int:
    count = 0
    for c in ServiceOverpassScraper().scrape(region):
        db.execute(
            "INSERT INTO service_raw (source_name, source_url, source_id, payload) "
...
def process_raw() -> dict[str, int]:
    ...
    for row in db.query("SELECT id, payload FROM service_raw WHERE NOT processed ORDER BY id"):
        rec = clean_service(_as_dict(row["payload"]))
        stats[_reconcile(rec)] += 1
        stats["processed"] += 1
        db.execute("UPDATE service_raw SET processed = true, processed_at = now() WHERE id = %s",
                   (row["id"],))
    return stats


def _reconcile(rec: dict) -> str:
    existing = db.query_one("SELECT * FROM services WHERE natural_key = %s", (rec["natural_key"],))
    if existing is None:
        _insert(rec)
        return "inserted"
    if not existing["is_active"] and not existing["is_claimed"]:
        db.execute("UPDATE services SET is_active = true, updated_at = now() WHERE id = %s",
                   (existing["id"],))
    diff = {f: rec[f] for f in _DIFF_FIELDS
            if rec.get(f) not in (None, "", [], {}) and _normalize(existing.get(f)) != _normalize(rec.get(f))}
    if not diff:
        db.execute("UPDATE services SET last_seen_at = now() WHERE id = %s", (existing["id"],))
        return "unchanged"
...
def _insert(rec: dict) -> None:
    ...
    row = db.query_one(
        f"INSERT INTO services ({', '.join(cols)}, last_seen_at) "
...
def _update(existing: dict, rec: dict, diff: dict) -> None:
    ...
    row = db.query_one(
        f"UPDATE services SET {assignments}, version = %s, updated_at = now(), "
...
def _snapshot(row: dict, reason: str) -> None:
    db.execute(
        "INSERT INTO service_versions (service_id, version, data, change_reason) "
        "VALUES (%s, %s, %s, %s)",
        (row["id"], row["version"], Jsonb(_jsonable(row)), reason),
    )
    if embeddings.enabled():
        db.execute("UPDATE services SET embedding = %s::vector WHERE id = %s",
                   (embeddings.to_vector_literal(embeddings.embed(embeddings.text_for(row))), row["id"]))


def deactivate_stale(days: int = 150) -> dict[str, int]:
    rows = db.query(
        "SELECT id FROM services WHERE deleted_at IS NULL AND is_active "
        "AND NOT is_claimed AND last_seen_at < now() - (%s || ' days')::interval", (days,))
    for r in rows:
        db.execute("UPDATE services SET is_active = false, version = version + 1, "
