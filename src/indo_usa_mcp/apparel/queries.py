...
def get_indian_apparel(
    *, lat: float | None = None, lng: float | None = None, radius_miles: float = 15.0,
    ...
    """List active Indian apparel/jewelry stores by geo-radius and/or filters."""
    ...
    sql = (f"SELECT {_COLS_SQL} FROM apparel WHERE {' AND '.join(where)} "
...
def search_apparel_by_text(
    ...
    sql = (f"SELECT {_COLS_SQL}, similarity(name, %s) AS match_score FROM apparel "
