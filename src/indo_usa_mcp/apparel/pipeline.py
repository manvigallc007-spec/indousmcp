"""Apparel pipeline: scrape -> raw -> clean/enrich/score -> canonical -> versioning."""
...
from .scraper import ApparelOverpassScraper
...
def clean_apparel(c: dict) -> dict:
    ...
    rec["tags"] = sorted(set(tagmod.extract("apparel", rec)) | set(c.get("extra_tags") or []))
    rec["description"] = describe.describe("apparel", rec)
...
def scrape_to_raw(region: str) -> int:
    count = 0
    for c in ApparelOverpassScraper().scrape(region):
        db.execute(
            "INSERT INTO apparel_raw (source_name, source_url, source_id, payload) "
...
def process_raw() -> dict[str, int]:
    ...
    for row in db.query("SELECT id, payload FROM apparel_raw WHERE NOT processed ORDER BY id"):
        rec = clean_apparel(_as_dict(row["payload"]))
        stats[_reconcile(rec)] += 1
        stats["processed"] += 1
        db.execute("UPDATE apparel_raw SET processed = true, processed_at = now() WHERE id = %s",
                   (row["id"],))
    return stats


def _reconcile(rec: dict) -> str:
    existing = db.query_one("SELECT * FROM apparel WHERE natural_key = %s", (rec["natural_key"],))
    if existing is None:
        _insert(rec)
        return "inserted"
    if not existing["is_active"] and not existing["is_claimed"]:
        db.execute("UPDATE apparel SET is_active = true, updated_at = now() WHERE id = %s",
                   (existing["id"],))
    diff = {f: rec[f] for f in _DIFF_FIELDS
            if rec.get(f) not in (None, "", [], {}) and _normalize(existing.get(f)) != _normalize(rec.get(f))}
    if not diff:
        db.execute("UPDATE apparel SET last_seen_at = now() WHERE id = %s", (existing["id"],))
        return "unchanged"
...
def _insert(rec: dict) -> None:
    ...
    row = db.query_one(
        f"INSERT INTO apparel ({', '.join(cols)}, last_seen_at) "
...
def _update(existing: dict, rec: dict, diff: dict) -> None:
    ...
    row = db.query_one(
        f"UPDATE apparel SET {assignments}, version = %s, updated_at = now(), "
...
def _snapshot(row: dict, reason: str) -> None:
    db.execute(
        "INSERT INTO apparel_versions (apparel_id, version, data, change_reason) "
        "VALUES (%s, %s, %s, %s)",
        (row["id"], row["version"], Jsonb(_jsonable(row)), reason),
    )
    if embeddings.enabled():
        db.execute("UPDATE apparel SET embedding = %s::vector WHERE id = %s",
...
def deactivate_stale(days: int = 120) -> dict[str, int]:
    rows = db.query(
        "SELECT id FROM apparel WHERE deleted_at IS NULL AND is_active "
        "AND NOT is_claimed AND last_seen_at < now() - (%s || ' days')::interval", (days,))
    for r in rows:
        db.execute("UPDATE apparel SET is_active = false, version = version + 1, "
