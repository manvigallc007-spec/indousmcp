"""Read queries backing the salon MCP capabilities."""
...
def get_indian_salons(
    *, lat: float | None = None, lng: float | None = None, radius_miles: float = 15.0,
    ...
    """List active Indian beauty salons by geo-radius and/or filters (incl. tag, open_now)."""
    ...
    sql = (f"SELECT {_COLS_SQL} FROM salons WHERE {' AND '.join(where)} "
...
def get_salon_details(salon_id: int) -> dict[str, Any] | None:
    record = db.query_one(
        f"SELECT {_COLS_SQL} FROM salons WHERE id = %s AND deleted_at IS NULL", (salon_id,))
    if record is None:
        return None
    record["version_history"] = db.query(
        "SELECT version, change_reason, changed_at FROM salon_versions "
        "WHERE salon_id = %s ORDER BY version DESC", (salon_id,))
    return record
...
def search_salons_by_text(
    ...
    sql = (f"SELECT {_COLS_SQL}, similarity(name, %s) AS match_score FROM salons "
...
def stats() -> dict[str, Any]:
    ...
    return {
        "raw_total": scalar("SELECT count(*) FROM salon_raw"),
        "raw_unprocessed": scalar("SELECT count(*) FROM salon_raw WHERE NOT processed"),
        "salons_active": scalar("SELECT count(*) FROM salons WHERE deleted_at IS NULL AND is_active"),
        "versions_total": scalar("SELECT count(*) FROM salon_versions"),
        "cities": db.query(
            "SELECT city, state, count(*) AS n FROM salons WHERE deleted_at IS NULL "
